(function () {

    "use strict";

    //khi debug thi co option nay, de lay http (non ssl)
    //window.debuglocal = 1;

    var
        libs = {

            shared: 'https://apphrm.github.io/media_shared'//'../media_shared'//
            ,

            vs2010: '192.168.1.91:5114'//'hellodat.pages.dev'//'hellodat.dnd.vn'// 'localhost:5114'//''/*debug*/// // //(1 in 2)     vs2010
            //mục đích lấy hole.html từ những nơi:
            //1. '' or localhost:5114 => lấy tại địa chỉ local debug
            //2. hellodat.dnd.vn => cho muc dich debug outside internet , vì github chấp nhận non-https
            //3. hellodat.pages.dev =>cloudflair chạy thật 
            //****** ngoài ra nó còn bị overide bởi quyery URL gởi qua từ trang chủ hellohrm.pages.dev
            ,

            srcpf$: 'http://192.168.1.91:10996'//'https://hrm.dnd.vn'//*debug*//
            //nơi lấy javascript + css
            ,

            testARG: JSON.stringify({ 'mnu': 'no_def_tab', 'arg': {} })//JS_nhapxuat no_def_tab //tmp_cakip_overview
            //tham số default tab open khi load pm

        }

        ,

        _utoa = function (s) {

            return window.btoa(unescape(encodeURIComponent(s)));

        }

        ,

        _atou = function (s) {
            return decodeURIComponent(escape(window.atob(s)));
        }
        ,

        cok = function (a, b, u) {

            u = u || 'Date';

            var exdate = new Date(), st_hn = window.location.hostname; exdate['set' + u](exdate['get' + u]() + b);
            document.cookie = a + '; domain=' + (/^((\d){1,3}\.){3}(\d){1,3}$/.test(st_hn) ? '' : '.') + st_hn.replace('www.', '') + (window.location.protocol.indexOf('https:') >= 0 ? '; SameSite=None; Secure' : '') + ';path=/ ; expires=' + exdate.toUTCString();
        }

        ,

        gQRY = function () {

            var QRY = window.location.search.substring(1),
                rst = {};

            // is there anything there ?
            //https://stackoverflow.com/questions/12381563/how-can-i-stop-the-browser-back-button-using-javascript
            if (QRY.length) {
                //
                //chi browser moi clean url
                if (window.location.protocol.indexOf('file:') != 0) {
                    // are the new history methods available ?
                    if (window.history != undefined && window.history.pushState != undefined) {
                        //remove url query string 
                        // if pushstate exists, add a new state to the history, this changes the url without reloading the page
                        window.history.pushState({}, document.title, window.location.pathname);
                        //
                    };
                };
                //
                //luu giu lai url query string :tham_so_demo
                localStorage.setItem('dGhhbV9zb19kZW1v', QRY);
                //
                //lot san language de khi xuong duoi dua vao key nay ma set cookie
                rst['lan_FLAG'] = 'vi';//default language
                //
            } else {
                //ko default key  rst['lan'] ******** khi load 
                QRY = localStorage.getItem('dGhhbV9zb19kZW1v') || '';
            };
            //
            //
            if (QRY.length > 0) {
                //
                var ps = decodeURIComponent(QRY).split(/\?|\&/);
                //
                for (var z = 0; z < ps.length; z++) {
                    if (ps[z]) {
                        //
                        var p = ps[z].split("=");
                        //
                        //quy uoc query string key dau tien la language
                        if (z == 0) {
                            //neu QRY lay tu localStorage set ko co set cookie khi chuyen doi ngon ngu
                            if (rst.hasOwnProperty('lan_FLAG')) {
                                if (p[0] == 'lan') {
                                    rst['lan_FLAG'] = p[1].trim();
                                    cok("selected_language=" + rst['lan_FLAG'], 365);
                                };
                            };
                            //
                        } else {
                            //
                            rst[p[0].trim()] = p[1] && p[1].trim();
                            //
                        };
                        //
                    };
                };
                //
            };
            //
            //delete rst['lan'];//xoa luon de khoi gay hau qua ve sau ..
            //
            return rst;
            //
        }(),


        rundemo = function (lg) {


            var csPrnt
                ,
                demoargs = st('getItem', 'ZGVtb2FyZ3M') || libs.testARG
                ,
                JOB = {

                    iniMSG: function () {

                        var def = $.Deferred(),
                            terMSG = $('<div style="margin:auto;width:500px;font-size:larger;"></div>').appendTo(app_container);

                        //debugger;
                        _gsC(srcpf$ + '/media/js/termynal.js', 'js', function () {

                            csPrnt = new Termynal(terMSG[0], {
                                cbNewLine: function () {
                                    //debugger;
                                    //terMSG[0].scrollTop = terMSG[0].scrollHeight;
                                }
                                , //lineDelay: 5
                                typeDelay: 40,
                                lineDelay: 700
                            });


                            //csPrnt.WriteLns(
                            //    [
                            //        { type: 'input', prompt: '▲', value: 'npm uninstall react' },
                            //        { value: 'Are you sure you want to uninstall \'react\'?' },
                            //        { type: 'input', typeDelay: 1000, prompt: '(y/n)', value: 'y' },
                            //        { type: 'progress', progressChar: '·' },
                            //        { value: 'Uninstalled \'react\'' },
                            //        { type: 'input', prompt: '▲', value: 'node' },
                            //        { type: 'input', prompt: '>', value: "Array(5).fill('🦄 ')" },
                            //        { value: "['🦄', '🦄', '🦄', '🦄', '🦄']" },
                            //        { type: 'input', prompt: '▲', value: 'cd ~/repos' },
                            //        { type: 'input', prompt: '▲ ~/repos', value: ' git checkout branch master' },
                            //        { type: 'input', prompt: '▲ ~/repos (master)', value: 'git commit -m \'Fix things\'' },
                            //    ]);


                            //csPrnt.WriteLn({ type: 'input', prompt: ' ▲ ', value: dog, typeDelay: 10 }, function (ln) {
                            //    //fnMSG();
                            //});

                            csPrnt.WriteLn({ type: 'input', prompt: '▲', value: 'Please wait a while we are establishing demo data.' }, function (ln) {
                                csPrnt.WriteLn({ type: 'progress', progressChar: '·' }, function (ln) {
                                    //fnMSG();
                                    def.resolve(csPrnt);
                                });
                            });

                        }, 'TermynalJS');
                        //
                        return def.promise();
                        //
                    }
                    ,
                    iniDAT: function () {
                        //
                        var def = $.Deferred(),


                            frm = libs.shared; //'https://qlns.pages.dev';

                        dbXLSX({

                            uri: '/api/headermaster', "act": 'GET', 'qry': 'demo', 'url': frm + '/odat/' + lg + '/assets/demo1.dat'

                        }).then(function (v1) {
                            //
                            def.resolve(v1);
                            //
                        }, function (error) {

                            def.resolve();

                        });
                        //
                        //
                        return def.promise();
                        //
                    }
                    ,
                    datEX: function () {

                        var def = $.Deferred();
                        //if (demoargs.mnu == 'JS_payrollonline') {
                        //    setTimeout(function () {

                        //        def.resolve(JOB.iniMSG());

                        //    }, 1000);
                        //} else {
                        //    def.resolve();
                        //};

                        //debug gia lap dash1 call 
                        //apisvr.__mr_imme = [
                        //                'https://hellohrm.github.io/utils/media/utils/attlog (1).dat',
                        //                new Date(2010, 1, 1, 0, 0, 0, 0),
                        //                new Date(2050, 1, 1, 0, 0, 0, 0),
                        //];





                        if (demoargs.mnu == 'JS_payrollonline') {
                            //
                            //khi land call online thi se url qry
                            if (gQRY['lan_FLAG']) {
                                if (demoargs.arg.att) {
                                    apisvr.__mr_imme = [
                                        demoargs.arg.att,
                                        new Date(2010, 1, 1, 0, 0, 0, 0),
                                        new Date(2050, 1, 1, 0, 0, 0, 0),
                                    ];
                                };
                            } else {
                                //user reload browser =>chi cho chay binh thuong
                                demoargs.arg = {};//
                            };
                            //
                        };

                        def.resolve();

                        return def.promise();
                    }
                };
            //
            if (demoargs) {
                //
                st('removeItem', 'ZGVtb2FyZ3M');
                //
                demoargs = JSON.parse(demoargs);
                //
                //------ dong y lay hoso nhan vien truoc ....
                $.when(JOB.iniDAT(), JOB.datEX()).then(function (cf, ex) {
                    //
                    var exe = function () {
                        //
                        //
                        $('#' + demoargs.mnu).trigger('click', { 'demo': btoa(JSON.stringify(demoargs.arg)) });
                        //
                    };
                    //
                    //can slide text ....
                    if (!ex && cf.inidat) {
                        ex = JOB.iniMSG();
                    };
                    //
                    //
                    $.when(cf.inidat, ex).then(function (f, x) {
                        if (x) {
                            csPrnt.WriteLn({ value: "['🦄', '🦄', '🦄', '🦄', '🦄']" }, function (ln) {
                                csPrnt.distroy();
                                exe()
                            });
                        } else {
                            exe();
                        };
                    });

                    //if (cf.inidat) {
                    //    //chua co nhan vien, va , ko phai import nham...
                    //    JOB.iniMSG().done(function (msg) {
                    //        //
                    //        $.when(cf.inidat).done(function (p) {

                    //            csPrnt.WriteLn({ value: "['🦄', '🦄', '🦄', '🦄', '🦄']" }, function (ln) {
                    //                csPrnt.distroy();
                    //                exe()
                    //            });

                    //        });
                    //        //
                    //    });
                    //} else {

                    //    exe();

                    //};
                });
                //
                //
            };
            //
        };



    if (gQRY['ts']) {//ts viet tat cua thamso
        //'ZGVtb2FyZ3M'  //demoargs



        var demoargs = JSON.parse(_atou(gQRY['ts']));

        libs.testARG = JSON.stringify(demoargs[1]); // JSON.stringify({ 'mnu': 'JS_nhapxuat', 'arg': {} });
        libs.shared = demoargs[0].shr;
        libs.srcpf$ = demoargs[0].jcs;
        libs.vs2010 = demoargs[0].fnc;
        //        libs = {

        //            shared: '../media_shared'//'https://apphrm.github.io/media_shared'//
        //        ,
        //            vs2010: 'hellodat.dnd.vn'//'hellodat.pages.dev' //'localhost:5114'//  //(1 in 2)     vs2010
        //        ,
        //            srcpf$: 'https://hrm.dnd.vn'//'http://192.168.1.91:10996'//

        //        ,

        //            testARG: JSON.stringify({ 'mnu': 'JS_nhapxuat', 'arg': {} })

        //        }
    };
    //debugger;
    //
    var div = document.createElement('div');

    div.innerHTML = '<link rel="stylesheet" type="text/css" href="https://hrm.dnd.vn/media/css/pm.css"/>' +

        '<iframe id="poorway" data-src="&lt;form&gt;&lt;input type=&quot;hidden&quot; name=&quot;whoami&quot; id=&quot;whoami&quot; data-whoareu=&quot;&quot; data-prefixsrc=&quot;' +

            'https://hrm.dnd.vn'


            +' &quot;/&gt;&lt;input type=&quot;hidden&quot; name=&quot;diesvr&quot; id=&quot;diesvr&quot; value=&quot;&quot; /&gt;&lt;/form&gt;"' +

        'data-lang=""' +

        'pages-dev="' + libs.vs2010 + '"' + //localhost:5114 (1 in 2) zkdat.pages.dev    vs2010

        'style="position:absolute;width:1px;height:1px;left:-10px" ></iframe> ';
    //
    div.getElementsByTagName('IFRAME')[0].onload = function () {
        //
        div = document.createElement('script');
        div.setAttribute('type', 'text/javascript');

        //
        div.src = libs.srcpf$ + "/media/js/pm.js"//"http://192.168.1.91:10996/media/js/pm.js"//"https://hrm.dnd.vn/media/js/pm.js";//(2 in 2)
        //
        //
        div.addEventListener('load', function () {
            //
            //
            admined = function (a, b, c) {
                //
                const lgDIC = $.extend(true, {}
                    //ko can vi vi no nam trong file hole.html
                    //,JSON.parse(atob(''))
                    ,
                    JSON.parse(atob(''))
                )

                    , lg = window['apisvr'] ? apisvr.a$.selected_language : 'vi'
                    , _gLA = function (na, ix) {
                        //
                        var re = lgDIC[lg] && lgDIC[lg][na] && lgDIC[lg][na];
                        //
                        if (ix === undefined || !re) return re;
                        //
                        return re[ix];
                    };
                //
                if (lgDIC.hasOwnProperty(lg)) {
                    for (var fuk in aLAN) {
                        if (lgDIC[lg].hasOwnProperty(fuk)) {
                            aLAN[fuk] = lgDIC[lg][fuk];
                        };
                    };
                };
                //
                $('#app').on('onTabAdding', function (e, tab) {
                    //debugger;

                    //var preLang = $(tab).find('pre#lang'),
                    //    dume = JSON.parse(atob(preLang.text()));

                    //preLang.text('');


                }).find('[lanf]').each(function (idx, el) {
                    //
                    var key = el.getAttribute('lanf'),
                        laN = $(el);
                    //
                    if (key.indexOf('lanCF') > -1) {
                        key = key.split('\x04');
                        laN.html(_gLA(key[1]));
                        //
                        laN.prev().html('<i class="flag-icon flag-icon-' + _gLA(0) + '"></i>');
                        //
                        //xu ly dropdown....
                        laN.parent().next().find('a').map(function (a, b) {
                            //
                            a = $(b);
                            key = a.attr('lang');
                            //
                            a.find('>i').css('visibility', lg == key ? 'visible' : 'hidden');
                            //
                            //render language item
                            a.find('span>i').next().html(_gLA(2, key));
                        });
                        //
                    } else if (key.indexOf('placeholder') > -1) {
                        //
                        key = key.split('\x04');
                        laN.attr(key[0], _gLA(key[1]));
                        //
                    } else {
                        laN.html(_gLA(key));
                    };
                    //
                });
                //
                //
                //xuất phát từ tmp_cakip_overview.html, dung cho chuc nang key customer
                if (window['__copyright']) {
                    //user click vao copyright o man hinh login
                    window['__copyright']();
                    return;
                }
                //
                //
                rundemo(lg);
                //
            };

        });

        document.body.appendChild(div);
    };
    //
    //div luc nay la iframe;
    document.body.appendChild(div);
    //
})();